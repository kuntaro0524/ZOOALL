...
class HEBI():
    def __init__(self, zoo, loop_measurement, logfile, stopwatch, phosec):
        ...
        # 500 um/sec is the limit at BL41XU
        self.limit_of_vert_velocity = 480.0  # [um] BL41XU
    ...
    def do2Dscan(self, scan_id, center, cond, phi_center):
        ...
        try:
            ...
            schfile, raspath = self.lm.rasterMaster(scan_id, scan_mode, center, vrange_um, hrange_um,
                                                    vstep_um, hstep_um, phi_center, cond)
            print schfile
    ...
    def doVscan(self, prefix, center, cond, phi):
        self.logfile.write("HEBI.doVscan: starts\n")
        # unit = [um]
        scan_hbeam = cond['ds_hbeam']
        scan_vbeam = cond['ds_vbeam']
        self.logger.info("HEBI.doVscan: beamsize %5.1f(H) x %5.1f(V) [um]" % (scan_hbeam, scan_vbeam))
        ...
        self.logger.info("Scan velocity: %8.3f[um/sec]" % vert_velocity)
    ...
    def doSingle(self, center_xyz, cond, phi_face, prefix):
        ...
        try:
            ...
            prefix = "single"
            multi_sch = self.lm.genMultiSchedule(phi_face, gonio_list, cond, self.phosec_meas, self.logfile, prefix=prefix)
            self.logfile.write("MultiSchedule class was used to generate the schedule file.\n")
            self.logfile.write("Data collection will be started by using %s.\n" % multi_sch)
            self.zoo.doDataCollection(multi_sch)
            self.zoo.waitTillReady()
        except Exception as e:
            self.logfile.write("Exception: %s\n" % e)
            self.logfile.write("HEBI.doSingle: ERRors occured in data collection loop.\n")
    ...
    def doHelical(self, left_xyz, right_xyz, cond, phi_face, prefix):
        ...
        try:
            ...
            if cry_y_len <= (2.0 * cond['ds_hbeam']):
                self.logfile.write("Crystal size is smaller than the horizontal beam size (%5.2f [um])\n" % cond['ds_hbeam'])
                self.logfile.write("Helical data collection is swithced to the single irradiation mode\n")
                self.doSingle(left_xyz, cond, phi_face, prefix, self.phosec_meas)
            else:
            ...
            self.logfile.write("Exception: %s\n" % e)
            self.logfile.write("HEBI.doHelical: ERRors occured in data collection loop.\n")
    ...
    def edgeCentering(self, cond, phi_face, rough_xyz, LorR = "Left"):
        ####################
        # Y(L) + 0.005 mm
        # Y(R) + 0.005 mm
        ####################
        ...
        print "HEBI.edgeCentering starts"
        self.logfile.write("HEBI.mainLoop: Vertical scan started.\n")
        self.logfile.flush()
        vertical_index = 0
        ...
        while (True):
            self.logfile.write("HEBI.mainLoop: %s vertical scan started.\n" % LorR)
            # Vertical scan
            phi_lv = phi_face + ref_rot
            scan_prefix = "%s%02d" % (prefix, vertical_index)
            ...
            self.logfile.write(
                "Left %s scan is started at %9.4f %9.4f %9.4f\n" % (LorR, mod_xyz[0], mod_xyz[1], mod_xyz[2]))
            scan_vert_path = self.doVscan(scan_prefix, mod_xyz, cond, phi_lv)
            try:
                new_xyz = self.anaVscan(scan_vert_path, scan_prefix, phi_lv, method="peak_xyz", isWeakScan=True)
                print "FOUND %s=" % prefix, new_xyz
                self.logfile.write(
                    "3D %s position : %9.4f %9.4f %9.4f\n" % (LorR, new_xyz[0], new_xyz[1], new_xyz[2]))
            except:
                print "Analyze vertical scans failed.\n"
                self.logfile.write("HEBI.mainLoop: %s vertical scan analysis failed.\n" % LorR)
                self.logfile.write("HEBI.mainLoop: increment Y coordinate by %8.4f mm\n" % self.gaburiyoru_h_length)
                vertical_index += 1
    ...
    def mainLoop(self, scan_path_2dface, scan_prefix_2dface, phi_face, cond, precise_face_scan=False):
        ...
        print "HEBI.mainLoop -> self.getSortedCryList"
        self.logfile.write(">>> HEBI.mainLoop.getSortedCryList\n")
        sorted_crylist = self.getSortedCryList(scan_path_2dface, scan_prefix_2dface, phi_face, isWeakScan=False)
        self.logfile.write("# of found crystals: %05d\n" % len(sorted_crylist))

        if len(sorted_crylist) == 0:
            self.logfile.write("No crystals were found\n")
            return 0

        print "Max hits analysis..."
        # the number of crystals
        n_max = cond['maxhits']
        self.logfile.write("# of Max crystals: %05d\n" % n_max)
        ...
        for crystal in sorted_crylist:
            ...
            if precise_face_scan == True:
                ...
                try:
                    ...
                    self.logfile.write("HEBI.mainLoop: R face scan failed.\n")
                    continue
                # Analyses of scanned map
                try:
                    left_face_xyz = self.ana2Dscan(left_face_path, lface_prefix, phi_face, method="left_lower",
                                                   isWeakScan=True)
                    self.logfile.write("Left  position precise 2D scan: %9.4f %9.4f %9.4f\n" % (
                    left_face_xyz[0], left_face_xyz[1], left_face_xyz[2]))
                except:
                    print "Analyze left scan failed."
                    self.logfile.write("HEBI.mainLoop: Left face scan failed.\n")
                    continue
                try:
                    right_face_xyz = self.ana2Dscan(right_face_path, rface_prefix, phi_face, method="right_upper",
                                                    isWeakScan=True)
                    self.logfile.write("Right position precise 2D scan: %9.4f %9.4f %9.4f\n" % (
                    right_face_xyz[0], right_face_xyz[1], right_face_xyz[2]))
                except:
                    print "Analyze left scan failed."
                    self.logfile.write("HEBI.mainLoop: Right face scan failed.\n")
                    continue
            # for large crystals
            else:
                self.logfile.write("HEBI.mainLoop: Precise scan is skipped.\n")
                print "Precise scan is skipped"
                left_face_xyz = lpos
                right_face_xyz = rpos
            ...
            self.logfile.write("Rough crystal size   = %5.2f [um]\n" % rough_crystal_um)
            self.logfile.write("Defined crystal size = %5.2f [um]\n" % cond['cry_min_size_um'])
            self.logfile.flush()
            #if rough_crystal_um <= (cond['ds_hbeam']):
            if rough_crystal_um <= cond['cry_min_size_um']:
                self.logfile.write("HEBI.mainLoop: %5.2f [um] crystal is smaller than %5.2f [um] limit.\n" % (
                    rough_crystal_um, cond['cry_min_size_um']))
                self.logfile.write("HEBI.mainLoop: Data collection is switched to 'single' irradiation mode.\n")
                ...
                self.logfile.write("HEBI.mainLoop: Center of the coordinate (%8.4f %8.4f %8.4f) was chosen\n" % (newx, newy, newz))
                print "From now edgeCentering starts"
                final_xyz = self.edgeCentering(cond, phi_face, center_xyz, LorR = "Left")
                self.doSingle(final_xyz, cond, phi_face, "single")

            else:
                print "Entering normal helical sequence."
                # Left edge vertical centering
                left_xyz = self.edgeCentering(cond, phi_face, left_face_xyz, LorR = "Left")
                # Right edge vertical centering
                right_xyz = self.edgeCentering(cond, phi_face, right_face_xyz, LorR = "Right")
