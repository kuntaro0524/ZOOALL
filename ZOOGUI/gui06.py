    ...
    def OnApply(self, event):
        ...
        for i in range(num):
            ...
            if i == 0: 
                self.log.Clear()
            if self.list.IsChecked(i):
                logstr= self.list.GetItemText(i,0)+self.list.GetItemText(i,1)+self.list.GetItemText(i,2)
                self.log.AppendText(logstr)
                sirname_new=self.list.GetItemText(i,0)
                givenname_new=self.list.GetItemText(i,1)
                nationarity_new=self.list.GetItemText(i,2)
                print "selected = ",sirname_new,givenname_new,nationarity_new
