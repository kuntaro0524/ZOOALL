...
class MyDialog(wx.Frame):
    def __init__(self, parent, list_selected, list_window):
        ...
        wx.Frame.__init__(self,parent,-1,"MYFRAME",size=(800,800))
        panel = wx.Panel(self)
        ...
        print "DDDDDDDDDD=",self.GetTitle()

        if self.instance.IsAnotherRunning():
            dialog = wx.MessageDialog(None, 'Already running %s ' % self.GetTitle(), 'Error', wx.ICON_ERROR)
            dialog.ShowModal()
            dialog.Destroy()
            wx.Exit()
    ...
    def PushSet(self, event):
        ...
        for i in self.list_sel:
            print "Changing",self.element_array[n]
