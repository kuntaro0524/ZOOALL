...
#logname = "/isilon/BL32XU/BLsoft/PPPP/10.Zoo/ZooLogs/ensoku_%s.log" % time_str
#logname = "ensoku.log"
#logging.config.fileConfig('/isilon/BL32XU/BLsoft/PPPP/10.Zoo/Libs/logging.conf', defaults={'logfile_name': logname})
#logging.config.fileConfig('ensoku_logging.conf', defaults={'logfile_name': logname})
#logger = logging.getLogger('ENSOKU')
...
# 8. dist_raster float,
# 9. dist_ds float,
# 10. cry_min_size_um float,
# 11. cry_max_size_um float,
# 12. loopsize float,
# 13. total_osc
# 14. osc_width
# 15. isDone
...
for p in ppp:
    ttt = (p['p_index'], p['isSkip'], p['isDS'], p['puckid'], p['pinid'], p['mode'],
           p['score_min'], p['score_max'], p['maxhits'], p['dist_ds'],p['total_osc'],p['osc_width'],
...
class Repository(wx.Frame):
    ...
    def setValues(self, index, param_list, isInitial=True):
        ...
        score_max, maxhits, dist_ds, total_osc, osc_width, \
        cry_min_size_um, cry_max_size_um, loopsize, n_mount, isDone, o_index = param_list
        ...
        print "index= %5d in setValues" % index, p_index
    ...
    def __init__(self, parent, id, title):
        ...
        self.list.InsertColumn(0, 'PriorityIndex', width=40)
        self.list.InsertColumn(1, 'isSkip', width=50)
        self.list.InsertColumn(2, 'puckID')
        self.list.InsertColumn(3, 'pinID')
        self.list.InsertColumn(4, 'Sample')
        self.list.InsertColumn(5, 'Scheme')
        self.list.InsertColumn(6, 'LoopSize')
        self.list.InsertColumn(7, 'MinScore')
        self.list.InsertColumn(8, 'MaxScore')
        self.list.InsertColumn(9, 'MaxHit')
        self.list.InsertColumn(11, 'DistRaster')
        self.list.InsertColumn(12, 'TotalOsc')
        self.list.InsertColumn(10, 'Osc width')
        self.list.InsertColumn(12, 'n_mount')
        self.list.InsertColumn(13, 'isDone')
        self.list.InsertColumn(14, 'OriginalIndex', width=40)
        ...
        for i in packages:
            """
            p['p_index'], p['isSkip'], p['isDS'], p['puckid'], p['pinid'], p['mode'],
            p['score_min'], p['score_max'], p['maxhits'], p['dist_ds'], p['total_osc'], p['osc_width'],
            p['cry_min_size_um'], p['cry_max_size_um'], p['loopsize'], p['n_mount'], p['isDone'], p['o_index']
            """
            p_index, isSkip, isDS, puckid, pinid, mode, score_min, \
            score_max, maxhits, dist_ds, total_osc, osc_width, \
            cry_min_size_um, cry_max_size_um, loopsize, n_mount, isDone, o_index = i
