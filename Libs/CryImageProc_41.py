...
class CryImageProc():
    ...
    def setImages(self, target_file, back_file):
        ...
        print "TRY1", target_file, back_file
    ...
    def findCenteringPoint_OLD(self, roi_xy, roi_pix_len, top_xy):
        # Half length of ROI (loop_size) in unit of [pix]
        half_roi_len_pix = int(roi_pix_len/2.0)
        # Loop top XY coordinates on image
        topx, topy = top_xy

        # horizontal coordinate for centering information
        if self.gonio_direction == "FROM_RIGHT":
            target_x = topx + half_roi_len_pix
            if target_x >= self.xmax:
                target_x = self.xmax

        # FROM_LEFT (BL45XU)
        else:
            target_x = topx - half_roi_len_pix
            if target_x < self.xmin:
                target_x = self.xmin

        width_x_ywidth=[]

        print "CIP.findCenteringPoint TOP XY =", top_xy
        print "CIP.findCenteringPoint horizontal position x=",target_x

        ymin = 1000
        ymax = 0

        # Sort along 'x' (horizontal axis on the OAV image)
        ...
        if ymax_flag == True and ymin_flag == True:
            ycenter = int((ymin + ymax)/2.0)
            # Log image
            if self.debug == True:
                baseimg = copy.deepcopy(self.timg)
                cv2.circle(baseimg, (target_x, ymin), 1, (255, 255, 0), 3)
                cv2.circle(baseimg, (target_x, ymax), 1, (255, 255, 0), 3)
                cv2.circle(baseimg, (target_x, ycenter), 1, (255, 255, 0), 3)
                cv2.imwrite("find_center.png", baseimg)
            return (target_x, ycenter)
        # Not found
        else:
            raise MyException("CIP.findCenteringPoint could not find any centering points.")
