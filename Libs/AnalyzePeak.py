...
class AnalyzePeak:
    ...
    def prepData3(self, col1, col2, col3):
        ...
        print len(xdat),len(y1dat),len(y2dat)
    ...
    def analyzeKnife(self, xlabel, ylabel, drvfile, outfig, comment="", opt="FWHM"):
        ...
        fwhm_ob, fwhm_sm, center_ob, center_sm, grav)
        peak = "Peak (%8.3f)" % maxx
        text = text + peak
    ...
    def analyzeAll(self, xlabel, ylabel, outfig, comment="", opt1="OBS", opt2="FCEN"):
        ...
        print "SMOOTH=",len(sy1),len(py1)
    
        # average
        ave = sy1.mean()
    
        # peak gravity
        grav = self.calcGrav(px, py1)
...
if __name__ == "__main__":
    ...
    px, py = ana.analyzeAll("dtheta1[pulse]", "Intensity", "test.png", "TESTTEST", "OBS", "PEAK")
