...
class KUMA:
    ...
    def getBestCondsMulti(self, cond, flux):
        ...
        self.logfile.write("Multi: Exposure time limit for dose %5.2f MGy = %10.5f " % (cond['dose_ds'], exptime_limit))
        self.logfile.write("Multi: Utilized flux = %5.2e " % flux)
        ...
        if mod_transmission >= 1.0:
            ...
            self.logfile.write("Exposure time was replaced by %8.3f sec\n" % exp_time)
            self.logfile.write("Measurement time will be longer than the initial condition\n")
            self.logfile.write("Initial data collection time: %8.2f [sec]\n" % (exp_orig * float(n_frames)))
            self.logfile.write("Current data collection time: %8.2f [sec]\n" % (exp_time * float(n_frames)))
    ...
    def getBestCondsHelical(self, cond, flux, dist_vec_mm):
        ...
        self.logfile.write("getBestCondsHelical starts\n")

        photon_density_limit = self.convDoseToDensityLimit(cond['dose_ds'], cond['wavelength'])
        dist_vec_um = dist_vec_mm * 1000.0  # [um]
        self.logfile.write("Flux density limit for dose %5.2f MGy= %5.2e " % (cond['dose_ds'], photon_density_limit))
        self.logfile.write("Utilized Beam = %5.2f x %5.2f [um]" % (cond['ds_vbeam'], cond['ds_hbeam']))
        self.logfile.write("Utilized flux = %5.2e [phs/sec]" % flux)
        ...
        # Attenuator is required
            ...
            exp_time = exp_orig
            self.logfile.write("Exposure time is input value: %8.2f [sec]\n" % exp_orig)
